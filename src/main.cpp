...
void HeavyTasks(void* pvParameters) {
    ...
    for (;;) {
        ...
        if (nextSchedule <= checkLastTime()) {
            for (;;) {
                String json = getArduinoDataJson("[" + getPositionJson(lat, lon, 0) + "," + getStadisticsJson(statFile) + "," + getStatusJson(String(flowProm)) + "]");
                Serial.println(json);
            ...
            statFile = "/Stadistics/" + getDate() + ".txt";
...
void FastSensors_Task(void* pvParameters) {
    ...
    for (;;) {
        ...
        if (buttonValue == 1) {
            ...
            /* if (flowSum < 0.2) {
                for (;;) {
                    // String json = getArduinoDataJson(getStatusJson("Perdida de flujo: " + String(flowProm)));
                    Serial.println(json);
