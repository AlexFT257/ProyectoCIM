...
#define ARDUINO_RX 16 // Connect to TX of the Serial MP3 Player module
#define ARDUINO_TX 17 // Connect to RX of the module



// Serial1 on the ESP32 can be used for communication with the Serial MP3 Player
...
{
  Serial.begin(9600);
  mp3Serial.begin(9600, SERIAL_8N1, ARDUINO_RX, ARDUINO_TX); // Initialize the hardware serial for MP3 Player communication
  delay(500);
...
// Function to send the play command with a filename
void sendPlayCommand(const char *filename)
{
  // Send the command to play a specific file by name
  mp3Serial.print("P");
  mp3Serial.print(filename);
...
void playHelloMp3()
{
  ...
  if (!played)
  {
    // Send the play command with the filename "hello.mp3"
    // sendPlayCommand("Hola.mp3");

    played = true; // Mark as played so it doesn't repeat continuously
  }
}
...
void loop()
{
  ...
  // Play "hello.mp3" by name

  // Set the volume to a medium level (adjust as needed)
  sendCommand(CMD_SET_VOLUME, 0x001E); // Volume level (0x001E is approximately 30 in decimal)
